...
try {
  serial_solver_sub_ = this->create_subscription<rm_interfaces::msg::SerialReceiveData>(
    ...
    std::bind(&ArmorSolverNode::serialReceiveDataCallback, this, std::placeholders::_1));
...
//NEW***************************************************************************************************************
void ArmorSolverNode::serialReceiveDataCallback(const rm_interfaces::msg::SerialReceiveData::SharedPtr msg) {
  // 处理接收到的串口数据
   ...
   //   RCLCPP_INFO(this->get_logger(), "Bullet speed: %.2f m/s", bullet_speed);
}
//********************************************************************************************************************
void ArmorSolverNode::timerCallback() {
  ...
  if (armor_target_.tracking) {
    try {
      control_msg = solver_->solve(armor_target_, this->now(), tf2_buffer_,bullet_speed);
    } catch (...) {
