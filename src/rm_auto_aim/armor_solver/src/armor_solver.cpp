...
//     serial_solver_sub_ = this->create_subscription<rm_interfaces::msg::SerialReceiveData>(
//         "serial/receive", rclcpp::SensorDataQoS(),
//         std::bind(&Solver::serialReceiveDataCallback, this, std::placeholders::_1));
// }

// } catch (const std::exception& e) {
//   RCLCPP_ERROR(this->get_logger(), "Failed to create subscriber: %s", e.what());
// }
  ...
  //-------------------------------------------------------------speed---------------------------------------------------------------//
  trajectory_compensator_->velocity = 22.0;
...
// void Solver::serialReceiveDataCallback(const rm_interfaces::msg::SerialReceiveData::SharedPtr msg) {
//   // 处理接收到的串口数据

//       bullet_speed = msg->bullet_speed;
      
//     //   solver_->setBulletSpeed();
//       // FYT_INFO("bullet_speed", "Current bullet_speed: %d", bullet_speed);
//       // std::cout<<"ok"<<std::endl;
//         RCLCPP_INFO(this->get_logger(), "Enter callback!"); 
// }

rm_interfaces::msg::GimbalCmd Solver::solve(const rm_interfaces::msg::Target &target,
                                            const rclcpp::Time &current_time,
                                            std::shared_ptr<tf2_ros::Buffer> tf2_buffer_,float speed) {
  ...
  if(speed == 0)
  {
   ...
   trajectory_compensator_->velocity = speed;
// 输出 bullet_speed（假设日志器已注册为 "armor_solver"）
  //FYT_DEBUG("armor_solver", "Bullet speed: {:.2f} m/s", speed);
  //std::cout<<speed<< std::endl;
    //FYT_DEBUG("armor_solver", "speed");
...
//***************************************************************************************************** */
  ...
  updateFilteredDistance(chosen_armor_position.norm());
...
//*********************************************************************************** */
  ...
  double cmd_pitch = pitch + pitch_offset + 0.055;
...
void Solver::updateFilteredDistance(double new_distance) {
        // 新数据入队
        distance_buffer_.push_back(new_distance);

        // 当队列超出窗口大小时，移除最早的数据
        if (distance_buffer_.size() > filter_window_size_) {
            distance_buffer_.pop_front();
        }

        // 计算滑动窗口内的平均值
        filtered_distance_ = std::accumulate(distance_buffer_.begin(), distance_buffer_.end(), 0.0) / distance_buffer_.size();
    }
...
bool Solver::isOnTarget(const double cur_yaw,
  ...
  // }
    ...
    if (std::abs(cur_yaw - target_yaw) < 0.005 &&
      std::abs(cur_pitch - target_pitch) < 0.1) {
    return true;
  }
