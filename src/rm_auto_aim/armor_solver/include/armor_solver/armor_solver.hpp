...
public:
  ...
  double getFilteredDistance() const {
    return filtered_distance_;
}

private:
void updateFilteredDistance(double new_distance);

  // 均值滤波相关成员变量
  const size_t filter_window_size_ = 5; // 滑动窗口大小，可根据实际需求调整
  std::deque<double> distance_buffer_; // 存储最近的distance值
  double filtered_distance_ = 0.0; // 滤波后的距离值

  //rclcpp::Subscription<rm_interfaces::msg::SerialReceiveData>::SharedPtr serial_solver_sub_;
  //void serialReceiveDataCallback(const rm_interfaces::msg::SerialReceiveData::SharedPtr msg);
