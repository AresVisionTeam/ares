...
std::vector<Armor> Detector::detect(const cv::Mat &input) noexcept {
  ...
  // std::vector<Armor> armors_vector;
  // Armor armor;

  // cv::Mat input_ = input.clone();

  // ov_detector->detect(input_,armor_objects_);

  // static std::string colorText[] = {"Blue", "Red", "Gray", "Purple"};
  // static std::string typeText[] = {"sentry", "1", "2", "3", "4", "5", "outpost", "base"};

  // if(!armor_objects_.empty()){
  //   armor_objects_.erase(
  //     std::remove_if(armor_objects_.begin(),armor_objects_.end(),
  //     [this](const ArmorObject &armor){
  //       int target_color;
  //       if(detect_color == fyt::EnemyColor::RED)
  //       {
  //         target_color = 0;
  //       }else if(detect_color == fyt::EnemyColor::BLUE)
  //       {
  //         target_color = 1;
  //       }else
  //       {
  //         target_color = 2;
  //       }
  //       return armor.color != target_color;
  //     }),armor_objects_.end()
  //   );
  // }

  // for(ArmorObject object : armor_objects_){
  //   armor.center = (object.apex[0] + object.apex[1] + object.apex[2] + object.apex[3] ) / 4;
  //   armor.type = fyt::auto_aim::ArmorType::SMALL;
  //   armor.number = typeText[object.cls];
  //   armor.left_light.top = object.apex[0];
  //   armor.left_light.bottom = object.apex[1];
  //   armor.right_light.top = object.apex[3];
  //   armor.right_light.bottom = object.apex[2];
  //   armor.left_light.center = (object.apex[0] + object.apex[1]) / 2;
  //   armor.right_light.center = (object.apex[3] + object.apex[2]) / 2;
  //   armor.left_light.axis = (object.apex[0] - object.apex[1]) / cv::norm((object.apex[0] - object.apex[1]));
  //   armor.right_light.axis = (object.apex[3] - object.apex[2]) / cv::norm((object.apex[3] - object.apex[2]));
  //   armor.left_light.length = cv::norm(object.apex[0] - object.apex[1]);
  //   armor.right_light.length = cv::norm(object.apex[3] - object.apex[2]);
  //   armor.left_light.tilt_angle = std::atan2(std::abs(armor.left_light.top.x - armor.left_light.bottom.x), std::abs(armor.left_light.top.y - armor.left_light.bottom.y)) / CV_PI * 180;
  //   armor.right_light.tilt_angle = std::atan2(std::abs(armor.right_light.top.x - armor.right_light.bottom.x), std::abs(armor.right_light.top.y - armor.right_light.bottom.y)) / CV_PI * 180;
  //   armor.confidence = object.prob;
  //   armor.classfication_result = fmt::format("{}:{:.1f}%", armor.number, object.prob * 100.0);
  //   armors_vector.emplace_back(armor);
  // }

  // armors_ = armors_vector;
