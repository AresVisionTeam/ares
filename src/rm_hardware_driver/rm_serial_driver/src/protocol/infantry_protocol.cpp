...
ProtocolInfantry::ProtocolInfantry(std::string_view port_name, bool enable_data_print) {
  ...
  send_packet_tool_ = std::make_shared<FixedPacketTool<16>>(uart_transporter);
  send_packet_tool_->enbaleDataPrint(enable_data_print);
  receive_packet_tool_ = std::make_shared<FixedPacketTool<20>>(uart_transporter);
  receive_packet_tool_->enbaleDataPrint(enable_data_print);
}
...
void ProtocolInfantry::send(const rm_interfaces::msg::GimbalCmd &data) {
  ...
  send_packet_tool_->sendPacket(packet);
}

bool ProtocolInfantry::receive(rm_interfaces::msg::SerialReceiveData &data) {
  FixedPacket<20> packet;
  if (receive_packet_tool_->recvPacket(packet)) {
  //  float bullet_speed = data.bullet_speed / 1000;
